...
def append_remotion_files(
    output_dir: Path,
    root_tsx: str,
    comp_tsx: str,
    all_remotion_scenes: list[dict],
) -> list[Path]:
    """
    Append new Remotion components and rebuild Root.tsx with ALL compositions.

    Unlike save_remotion_files (which overwrites), this function:
    1. Appends new component code to the existing MyComp.tsx
    2. Rebuilds Root.tsx to include compositions for ALL remotion scenes

    Args:
        output_dir: The output directory for this video job
        root_tsx: Ignored (Root.tsx is rebuilt from all_remotion_scenes)
        comp_tsx: The new component TSX code to append
        all_remotion_scenes: ALL scenes that need Remotion rendering (original + fallback)
    """
    remotion_dir = output_dir / "remotion"
    remotion_dir.mkdir(parents=True, exist_ok=True)
    ...
    # 1. Append component code to MyComp.tsx
    comp_path = remotion_dir / "MyComp.tsx"
    if comp_path.exists():
        existing = comp_path.read_text(encoding="utf-8")
        # Avoid duplicate imports — strip import lines from new code
        new_lines = []
        for line in comp_tsx.splitlines():
            stripped = line.strip()
            if stripped.startswith("import ") and stripped in existing:
                continue  # Skip duplicate imports
            new_lines.append(line)
        merged = existing.rstrip() + "\n\n// --- Fallback component ---\n" + "\n".join(new_lines)
        comp_path.write_text(merged, encoding="utf-8")
    else:
        comp_path.write_text(comp_tsx, encoding="utf-8")
    paths.append(comp_path)

    # 2. Rebuild Root.tsx with ALL compositions
    compositions = ""
    imports_set = []
    for s in all_remotion_scenes:
        compositions += f"""
            <Composition
                id="{s['comp_id']}"
                ...
                fps={{30}}
            />"""
        if s['comp_name'] not in imports_set:
            imports_set.append(s['comp_name'])

    imports = ", ".join(imports_set)
    combined_root = f"""import {{Composition}} from 'remotion';
...
"""
    root_path = remotion_dir / "Root.tsx"
    root_path.write_text(combined_root, encoding="utf-8")
    paths.append(root_path)

    print(f"   Appended component to MyComp.tsx, rebuilt Root.tsx with {len(all_remotion_scenes)} compositions")
    return paths
